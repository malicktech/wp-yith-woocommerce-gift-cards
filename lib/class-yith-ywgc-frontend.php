...
if ( ! class_exists( 'YITH_YWGC_Frontend' ) ) {
	...
	class YITH_YWGC_Frontend {
		...
		protected function __construct() {
			...
			add_action( 'yith_ywgc_show_gift_card_amount_selection', array(
				$this,
        ...
        public function frontend_init() {
            ...
            if ( get_option( 'ywgc_gift_card_form_on_cart' ) != 'no' ){

                $ywgc_cart_hook = apply_filters( 'ywgc_gift_card_code_form_cart_hook', ( empty( get_option( 'ywgc_gift_card_form_on_cart_place' ) ) ? 'woocommerce_before_cart' : get_option( 'ywgc_gift_card_form_on_cart_place' ) ) );
                /**
            ...
            if ( get_option( 'ywgc_gift_card_form_on_checkout' ) != 'no' ){

                $ywgc_checkout_hook = apply_filters( 'ywgc_gift_card_code_form_checkout_hook', empty( get_option( 'ywgc_gift_card_form_on_checkout_place' ) ) ? 'woocommerce_before_checkout_form' : get_option( 'ywgc_gift_card_form_on_checkout_place' ) ) ;
                /**
                 * Show the gift card section for entering the discount code in the cart page
                 */
