...
if ( ! class_exists( 'YITH_YWGC_Backend_Premium' ) ) {
    ...
    class YITH_YWGC_Backend_Premium extends YITH_YWGC_Backend {
        ...
        public function manage_bcc( $gift_card ) {
            ...
            if ( get_option ( "ywgc_blind_carbon_copy" ) == "yes" ) {
                $recipients[] = get_option( 'admin_email' );
            }

            //  Check if the option is set to add the gift card buyer email
            if ( get_option ( "ywgc_blind_carbon_copy_to_buyer" ) == "yes" ) {
        ...
        public function update_totals_on_save_order_items( $order_id, $items ){
            ...
            if( isset( $items['order_status'] ) && $items[ 'order_status' ] == 'wc-refunded' ) return;
            $order = wc_get_order( $order_id );
        ...
        public function yith_ywgc_transform_smart_coupons_buttons(){
            ...
            if ( class_exists( 'WC_Smart_Coupons' ) ) {
                ...
                <tr id="ywgc_ajax_zone_transform_smart_coupons">
                    <td width="20%">
                        <ul>
                            <li><strong><?php esc_html_e( 'Transform Smart Coupons to YITH Gift Cards ', 'yith-woocommerce-gift-card' );?></strong></li>
                        </ul>
                    </td>
                    <td>
                        <ul>
                            <li><label><a class="ywgc_transform_smart_coupons_class button" id="yith_ywgc_transform_smart_coupons" >Transform</a></label><?php echo wc_help_tip( __( 'This will transform your Smart Coupons to Gift Cards', 'yith-woocommerce-gift-card' ) ); ?></li>
                        </ul>
                        <span class="description"><?php esc_html_e( 'This action cannot be undone', 'yith-woocommerce-gift-card' ); ?></span>
                    </td>
                </tr>
                <?php
            }
