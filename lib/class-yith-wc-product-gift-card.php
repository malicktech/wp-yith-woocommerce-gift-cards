...
if ( ! class_exists ( 'WC_Product_Gift_Card' ) ) {
	...
	class WC_Product_Gift_Card extends WC_Product {
		...
		public function add_to_cart_text() {
            $text = $this->is_purchasable () && $this->is_in_stock () ? apply_filters( 'yith_ywgc_select_amount_text' , __( 'Select amount', 'yith-woocommerce-gift-cards' ) ) : __ ( 'Read more', 'yith-woocommerce-gift-cards' );
		...
		public function get_manual_header_image( $size = 'full', $return = 'url' ) {
			...
			$image_id = yit_get_prop ( $this->get_product_instance(), self::YWGC_PRODUCT_IMAGE );

			$image_id = ( $image_id != '' ) ? $image_id : get_post_thumbnail_id( yit_get_prop($this->get_product_instance (),'id') );
