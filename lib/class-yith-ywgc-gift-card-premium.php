...
if ( ! class_exists( 'YWGC_Gift_Card_Premium' ) ) {
	...
	class YWGC_Gift_Card_Premium extends YITH_YWGC_Gift_Card {
		...
		public function get_gift_card_error( $err_code ) {
			...
			switch ( $err_code ) {
				case self::E_GIFT_CARD_NOT_EXIST:
					$err = sprintf( __( 'The gift card code "%s" does not exist!', 'yith-woocommerce-gift-cards' ), $this->gift_card_number );
					break;
				case self::E_GIFT_CARD_NOT_YOURS:
					$err = sprintf( __( 'Sorry, it seems the gift card code "%s" is not for you and cannot be applied to your cart.', 'yith-woocommerce-gift-cards' ), $this->gift_card_number );
					break;
				case self::E_GIFT_CARD_ALREADY_APPLIED:
					$err = sprintf( __( 'The gift card code "%s" is already applied!', 'yith-woocommerce-gift-cards' ), $this->gift_card_number );
					break;
				case self::E_GIFT_CARD_EXPIRED:
					$err = sprintf( __( 'Sorry, the gift card code "%s" is expired and cannot be used.', 'yith-woocommerce-gift-cards' ), $this->gift_card_number );
					break;
				case self::E_GIFT_CARD_DISABLED:
					$err = sprintf( __( 'Sorry, the gift card code "%s" is currently disabled and cannot be used.', 'yith-woocommerce-gift-cards' ), $this->gift_card_number );
					break;
				case self::E_GIFT_CARD_DISMISSED:
					$err = sprintf( __( 'Sorry, the gift card code "%s" is no longer valid!', 'yith-woocommerce-gift-cards' ), $this->gift_card_number );
					break;
				case self::E_GIFT_CARD_INVALID_REMOVED:
					$err = sprintf( __( 'Sorry, it seems the gift card code "%s" is invalid - it has now been removed from your cart.', 'yith-woocommerce-gift-cards' ), $this->gift_card_number );
					break;
		...
		public function get_gift_card_message( $err_code ) {
			...
			switch ( $err_code ) {
				...
				case self::GIFT_CARD_SUCCESS:
					$err = __( 'Gift card code applied successfully.', 'yith-woocommerce-gift-cards' );
					break;
				case self::GIFT_CARD_REMOVED:
					$err = __( 'Gift card code removed successfully.', 'yith-woocommerce-gift-cards' );
					break;
			}
		...
		public function set_as_sent() {
			...
			$this->delivery_send_date = current_time( $date_format, 0 );
