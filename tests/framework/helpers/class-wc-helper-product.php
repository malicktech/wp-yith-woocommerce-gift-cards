...
class WC_Helper_Product {
     ...
     */
    public static function create_gift_card_product( $type ) {
        // Create the product
        $product_id = wp_insert_post( array(
            'post_title'  => 'Gift card ' . $type . ' - ' . uniqid(),
            'post_type'   => 'product',
            'post_status' => 'publish',
        ) );
        ...
        update_post_meta( $product_id, '_sku', 'sku_' . uniqid() );
        ...
        wp_set_object_terms( $product_id, 'gift-card', 'product_type' );

        $gift_product  = new WC_Product_Gift_Card( $product_id );

        $gift_product->add_amount ( '10' );
        $gift_product->add_amount ( '20' );
        $gift_product->add_amount ( '30' );
	 ...
	 */
	public static function create_simple_product( $price ) {
		// Create the product
